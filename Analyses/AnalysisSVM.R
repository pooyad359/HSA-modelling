...
#FUNCTIONS
{
  my.predict<-function(model,data,batch.size=NULL){
    if(is.null(batch.size)){
      batch.size<-floor(nrow(data)/100)
      if(batch.size<1){
        batch.size=1
      }
    }
    pred<-data[,1]*0
    nbatch<-ceiling(nrow(data)/batch.size)
    for(i in 1:nbatch){
      i1<-(i-1)*batch.size+1
      i2<-i*batch.size
      if(i==nbatch){
  ...
  add.col<-function(data){
    ...
    output$DenEst2<-4.6333+76.876*output$DenEst-20.776*(output$DenEst)^2
...
library(Cubist)
{
  ...
  tsamp$target<-ttarg
  vsamp$target<-vtarg
  #tsamp<-norm.time(tsamp)
  #vsamp<-norm.time(vsamp)##<------------------------
  #cubistControl(extrapolation = 10)
  fitsvm<-svm(target~.,tsamp)
  
  pred1<-my.predict(fitsvm,tsamp)
  print(rmse(pred1,ttarg))
  pred2<-my.predict(fitsvm,vsamp)
  print(rmse(pred2,vtarg))
}
hist(pred2,breaks = 200)
my.summary(wtr.data[[29]])
summary(wtr.data[[29]])
predxxx<-my.predict(fitsvm,tsamp[1:10000,])
#plot(ttarg,pred1)
...
# fit2$coefficients
{
  ...
  #tar.pred<-predict(fitsvm,testdat)
  tar.pred<-my.predict(model=fitsvm,data=testdat)
  summary(tar.pred)
  sum(is.na(tar.pred))
  ind<- is.na(tar.pred)|tar.pred>60 | tar.pred<40 
  summary(ind)
  hist(tar.pred,breaks = 500)
  tar.pred[ind]<-53.5
  {
    ind<- is.na(tar.pred) 
    tar.pred[ind]<-53.5
  ...
  write.csv(tar.pred,file="submissionSVM.csv",row.names = F)
